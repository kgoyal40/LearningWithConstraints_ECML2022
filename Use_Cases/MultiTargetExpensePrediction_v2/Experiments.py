    ...
    if params is None:
        ...
        while True:
            ...
            final_model.learn(X, y, M=M, K=K, validation_set=None,
                              batch_size=batch_size, epochs=epoch, learning_rate=learning_rate, learner='fumalik',
                              step_size=1, waiting_period=5, early_stopping=False)
        ...
        custom_metric_train_1 = calculate_constraint_penalty(X, prediction_train[0], K)

        custom_metric_train_2 = calculate_custom_metric(X, y, prediction_train[0], K)
        ...
        custom_metric_test_1 = calculate_constraint_penalty(test_data[0], prediction_test[0], K)

        custom_metric_test_2 = calculate_custom_metric(test_data[0], test_data[1], prediction_test[0], K)
        ...
        output.columns = ['fold', 'margin/alpha', 'knowledge', 'learning_rate', 'batch_size', 'epochs', 'runtime',
                          'weights', 'violations_train_data', 'total_mse_train', 'constraint_penalty_train',
                          'custom_metric_train_2', 'violations_train_prediction', 'violations_test_data',
                          'total_mse_test', 'constraint_penalty_test', 'custom_metric_test_2',
                          'violations_test_prediction', 'counter_examples_found']

        return output, CV_log, fold_train_losses, val_evaluation, violations_test_all[1]
...
def single_baseline_run_2(X, y, test_data=None, params=None, fold_index=None):
    ...
    for alpha in alphas:
        ...
        custom_metric_train_1 = calculate_constraint_penalty(X_train, prediction_train[0], K)
        custom_metric_train_2 = calculate_custom_metric(X_train, y_train, prediction_train[0], K)

        custom_metric_val_1 = calculate_constraint_penalty(X_val, prediction_val[0], K)
        custom_metric_val_2 = calculate_custom_metric(X_val, y_val, prediction_val[0], K)
        ...
        print([fold_index, alpha, K, '-', '-', '-', runtime,
    ...
    output.columns = ['fold', 'margin/alpha', 'knowledge', 'learning_rate', 'batch_size', 'epochs', 'runtime',
                      ...
                      'violations_val_data', 'total_mse_val', 'constraint_penalty_val',
                      'custom_metric_val_2', 'violations_val_prediction', 'violations_test_data',
                      'total_mse_test', 'constraint_penalty_test', 'custom_metric_test_2', 'violations_test_prediction',
