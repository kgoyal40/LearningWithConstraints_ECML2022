...
def find_counter_example(X, parameters=None, K=None, num=10):
    ...
    if K is not None:
        ...
        Fs.append(Or(Sum([MultiLinearRegressionLearner.sumproduct(_X, parameters[j])
                          for j in range(len(parameters))]) > 2*((_X[-1] - K[0]) / K[1]) + 0.01,
...
def calculate_constraint_penalty(X, y_pred, K):
    ...
    for i in range(X.shape[0]):
        ...
        Penalty_Constraint_1 += max(0, sum(pred) - 2*transformed_household_income[i])
...
def calculate_custom_metric(X, y, y_pred, K):
    ...
    for i in range(X.shape[0]):
        ...
        pred = list(map(lambda x: x[i], y_pred))
        if (sum(true) <= 2*transformed_household_income[i]) and (true[1] <= 0.05 * transformed_household_income[i]):
...
def count_violations(X, y, K=None):
    ...
    for i in range(X.shape[0]):
        if (sum([y[j][i] for j in range(len(y))]) > 2*((X['Total Household Income'][i] - K[0]) / K[1]) + 0.01) or \
