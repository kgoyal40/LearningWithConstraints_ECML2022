...
def single_run(X, y, M, K, test_set, batch_size, epoch, learning_rate, fold):
    ...
    if len(model.training_losses) > 100:
        val_loss = model.calculate_gradient(test_set[0], test_set[1])[1]
        # f = open("CV_log.txt", "a+")
        # f.write('cv_log_entry {} {}\n'.format(learning_rate, fold))
        # f.close()
...
def cross_validate_multiclass(X, y, params=None, n_folds=5):
    if params is None:
