...
def single_model_run(X, y, test_data=None, params=None, fold_index=None):
    ...
    if params is None:
        ...
        correct_prediction_train = [[1 if y[i] == prediction_train[0][i] else 0][0]
                                    if (X['group_The Beatles'][i] == 0) or
                                       ((X['group_The Beatles'][i] == 1) and
                                        (y[i] in ['Pop', 'Rock']))
                                    else 0
                                    for i in range(X.shape[0])]

        corrected_train_accuracy = sum(correct_prediction_train) / X.shape[0]
        modified_train_accuracy = calculate_modified_accuracy(X, y, prediction_train[0])

        correct_prediction_test = [[1 if test_data[1][i] == prediction_test[0][i] else 0][0]
                                   if (test_data[0]['group_The Beatles'][i] == 0) or
                                      ((test_data[0]['group_The Beatles'][i] == 1) and
                                       (test_data[1][i] in ['Pop', 'Rock']))
                                   else 0
                                   for i in range(test_data[0].shape[0])]

        corrected_test_accuracy = sum(correct_prediction_test) / test_data[0].shape[0]
        modified_test_accuracy = calculate_modified_accuracy(test_data[0], test_data[1], prediction_test[0])
        ...
        output = output.append([[fold_index, M, K, learning_rate, batch_size, epoch, final_model.runtime,
                                 ...
                                 modified_train_accuracy, violations_train_prediction, violations_test_data,
                                 corrected_test_accuracy, modified_test_accuracy, violations_test_prediction,
                                 len(counter_examples), len(final_model.training_losses)]])
...
def single_baseline_run_2(X, y, test_data=None, params=None, fold_index=None):
    ...
    for l in loss_types:
        for alpha in alphas:
            ...
            correct_prediction_train = [[1 if y_train[i] == y_train_b[i] else 0][0]
                                        if (X_train['group_The Beatles'][i] == 0) or
                                           ((X_train['group_The Beatles'][i] == 1) and
                                            (y_train[i] in ['Pop', 'Rock']))
                                        else 0
                                        for i in range(X_train.shape[0])]

            corrected_train_accuracy = sum(correct_prediction_train) / X_train.shape[0]
            modified_train_accuracy = calculate_modified_accuracy(X_train, y_train, y_train_b)

            correct_prediction_val = [[1 if y_val[i] == y_val_b[i] else 0][0]
                                      if (X_val['group_The Beatles'][i] == 0) or
                                         ((X_val['group_The Beatles'][i] == 1) and
                                          (y_val[i] in ['Pop', 'Rock']))
                                      else 0
                                      for i in range(X_val.shape[0])]

            corrected_val_accuracy = sum(correct_prediction_val) / X_val.shape[0]
            modified_val_accuracy = calculate_modified_accuracy(X_val, y_val, y_val_b)

            correct_prediction_test = [[1 if test_data[1][i] == y_test_b[i] else 0][0]
                                       if (test_data[0]['group_The Beatles'][i] == 0) or
                                          ((test_data[0]['group_The Beatles'][i] == 1) and
                                           (test_data[1][i] in ['Pop', 'Rock']))
                                       else 0
            ...
            corrected_test_accuracy = sum(correct_prediction_test) / test_data[0].shape[0]
            modified_test_accuracy = calculate_modified_accuracy(test_data[0], test_data[1], y_test_b)
            ...
            output = output.append([[fold_index, l, alpha, K, '-', '-', '-', runtime,
                                     ...
                                     modified_train_accuracy, violations_train_prediction, violations_val_data,
                                     corrected_val_accuracy, modified_val_accuracy, violations_val_prediction,
                                     violations_test_data, corrected_test_accuracy, modified_test_accuracy,
