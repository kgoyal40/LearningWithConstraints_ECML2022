...
class Grid():
    def __init__(self, n, nodes):
        ...
        for e in range(n * (n - 1) * 2):
            if e < n * (n - 1):
                edgemap[
                    (self.nodes[n * (e / (n - 1)) + e % (n - 1)], self.nodes[n * (e / (n - 1)) + e % (n - 1) + 1])] = e
                edgemap[
                    (self.nodes[n * (e / (n - 1)) + e % (n - 1) + 1]), self.nodes[n * (e / (n - 1)) + e % (n - 1)]] = e
                edgemap[e] = (n * (e / (n - 1)) + e % (n - 1), n * (e / (n - 1)) + e % (n - 1) + 1)
            else:
                e -= n * (n - 1)
                edgemap[(self.nodes[n * (e % (n - 1)) + e / (n - 1)],
                         self.nodes[n * (e % (n - 1) + 1) + e / (n - 1)])] = e + n * (n - 1)
                edgemap[(self.nodes[n * (e % (n - 1) + 1) + e / (n - 1)],
                         self.nodes[n * (e % (n - 1)) + e / (n - 1)])] = e + n * (n - 1)
    ...
    def bfs(self, start, end):
        ...
        while queue:
            ...
            if curr[-1] == end:
                sols.append(curr)
                continue
            # If we've found a solution already and we've passed the length of that solution, time to convert to
            # edges and quit
    ...
    def toEdges(self, sols):
        ...
        return [[self.edgemap[(l[i], l[i + 1])] for i in range(len(l) - 1)] for l in sols]
...
def gen_grid_removed(n, removed):
    """
    Generate a nxn grid, with the listed edge indices removed. The edges are ordered from left to right, then top to
    bottom, then the same thing sideways.
    """
    grid = gen_grid(n)
...
def gen_grid(n):
    """ Generate nxn grid """
    rows = [[Node(j + n * i) for j in range(n)] for i in range(n)]
